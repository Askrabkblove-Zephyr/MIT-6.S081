...
![image-20251107191325765](C:\Users\wills\AppData\Roaming\Typora\typora-user-images\image-20251107191325766.png)
...
# git：下载源代码工具
# build-essential：一个基础编译工具套件的元数据包
# gdb-multiarch：GNU 调试器的多架构版本。gdb-multiarch 可以调试多种 CPU 架构的程序，包括 RISC-V。你需要用它来调试运行在 RISC-V 架构上的 xv6 内核。
# qemu-system-misc：一个通用的硬件模拟器和虚拟化软件。QEMU 可以模拟出一台 RISC-V 架构的虚拟计算机，让你能在自己的电脑上启动并运行 xv6。
# gcc-riscv64-linux-gnu：针对 RISC-V 64 位 架构的 GCC 交叉编译器。这个特殊的 GCC 则能让你在 x86 电脑上编译出能在 RISC-V 机器上运行的程序（即 xv6）
# binutils-riscv64-linux-gnu：针对 RISC-V 64 位架构的二进制工具集。它包含了处理 RISC-V 目标文件所需的辅助工具，如 objdump (反汇编)， objcopy， ld (链接器) 等，与上面的交叉编译器配套使用。
sudo add-apt-repository universe # 添加软件源，universe是由社区维护的、开源且免费的软件。
