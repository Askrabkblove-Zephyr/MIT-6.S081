...
runcmd（[user/sh.c:58](https://github.com/mit-pdos/xv6-riscv/blob/riscv/user/sh.c#L58)）运行实际的命令。对于“echo hello”，它会调用exec（[user/sh.c:78](https://github.com/mit-pdos/xv6-riscv/blob/riscv/user/sh.c#L78)）。如果exec成功，那么子进程将执行echo中的指令，而不是runcmd。在某个时候，echo会调用exit，这将导致父进程从main中的wait返回（[user/sh.c:145](https://github.com/mit-pdos/xv6-riscv/blob/riscv/user/sh.c#L145)）。
